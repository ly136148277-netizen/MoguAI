...
2. **v1.5.5** 安全热修 + 打包边界收口（基线；`v1.5.4` 安装包已 yank）
3. **v1.6.0-alpha**：先连接「用户已安装并运行的 Gateway」（不做自动安装）→ 任务模型 → 流式 Run → 任务中心 → 双轨对话
4. **降级铁律**：请求已被 Gateway 接受后，仅等待超时**不得**自动降级重发（防双执行）
5. **v1.7** 按 §5 四件套交付 Skills，禁止「仅 SKILL.md」；外部渠道更后
|
