...
- 浏览器：`act` / `click` / `fill` / `extract`（Playwright 外置 steps）
- 记忆：`preference` / `project` / `session` 分层；大脑结束自动沉淀高价值事实
- 信任文案：权限弹窗人话 + 下一步建议；任务详情「下一步建议」

## P2（第 12 项）

- MCP：设置里推荐卡片 + 表单列表 + 测连通；高级 JSON 仍可用
- 更新：设置显示更新源 / 当前版本 / 代码签名状态与启用清单（证书未配时保持未签名流水线）
