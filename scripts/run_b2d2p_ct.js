...
const SMOKE_PREFIX = "ct-b2d2p-smoke-django11019";

const CT_JOBS = [
  { instance: "django__django-13265", prefix: "ct-b2d2p-django13265" },
  { instance: "django__django-12497", prefix: "ct-b2d2p-django12497" },
  { instance: "django__django-11019", prefix: "ct-b2d2p-django11019" },
  { instance: "django__django-15695", prefix: "ct-b2d2p-django15695" },
];
...
function runBench({ runId, instance, work, withEval }) {
  ...
  if (fs.existsSync(dir) && !(withEval && isComplete(runId))) {
    // Smoke always wipe; CT wipe incomplete only.
    if (!withEval || !isComplete(runId)) {
      fs.rmSync(dir, { recursive: true, force: true });
      console.log(`[b2d2p] removed ${withEval ? "incomplete " : ""}${runId}`);
    }
  }
  ...
  const env = {
    ...
    MOGU_D2_CYCLE_ARTIFACT_DIR: path.join(dir, "d2_cycles"),
  };
  ...
  console.log(
    `[b2d2p] flags: STRUCTURED_RETRY=1 HYPOTHESIS_DIVERSITY=1 model=${env.MOGU_BENCH_MODEL}`
  );
  ...
  console.log(`[b2d2p] ${runId} exit=${code}`);
  return code;
}

function runRejectProbe() {
  console.log("\n======== reject probe (artificial duplicate path) ========");
  const r = spawnSync(process.execPath, ["scripts/probe_b2d2p_reject.js"], {
    cwd: ROOT,
    stdio: "inherit",
    windowsHide: true,
  });
...
function runSmokeJudge(runId) {
  ...
  const r = spawnSync(
    process.execPath,
    ["scripts/judge_b2d2p_smoke.js", "--run-id", runId],
...
function printHelp() {
  console.error(`B2-D2′ runner — choose exactly one mode:

  node scripts/run_b2d2p_ct.js --smoke-only
      Live mechanism smoke on ${SMOKE_INSTANCE} (no --eval, Branch N/A)

  node scripts/run_b2d2p_ct.js --ct
      Formal CT: 4 instances × k=3 + official eval
...
if (smokeOnly === formalCt) {
  ...
  if (smokeOnly && formalCt) {
    console.error("[b2d2p] refuse: pass only one of --smoke-only | --ct");
  } else {
    printHelp();
  }
  process.exit(2);
...
if (smokeOnly) {
  const rejectCode = runRejectProbe();
  if (rejectCode !== 0) {
    console.error("[b2d2p] reject probe FAIL — fix gate before live smoke");
    process.exit(rejectCode);
  }
  ...
  console.log(`\n===== B2-D2′ LIVE SMOKE =====`);
  console.log(`D2′ mechanism smoke: ${code === 0 && judgeCode === 0 ? "PASS" : "FAIL"}`);
